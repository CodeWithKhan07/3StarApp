...
export function InvoicesScreen({pendingOnly=false}:{pendingOnly?:boolean}){
  const {data,updateInvoiceStatus,completeInvoicePayment,deleteRecord}=useBusinessData();const [query,setQuery]=useState("");const [status,setStatus]=useState("all");const [error,setError]=useState("");
  ...
  return <><PageHeader title={pendingOnly?"Pending Payments":"Invoices & Payments"} description={pendingOnly?"Follow up outstanding balances.":"Create invoices from quotations or without a quotation, then manage payments."} actions={!pendingOnly?<Link className="button button--primary" href={routes.newInvoice}><Plus size={14}/>New Invoice</Link>:undefined}/>
