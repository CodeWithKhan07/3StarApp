  ...
  return <>
    ...
    <section className="statement-company-list">
      {groups.length ? groups.map((group) => <article className="card statement-company-card" key={normalize(group.companyName)}>
        ...
        <div className="table-wrap"><table className="data-table statement-table"><thead><tr><th>Date</th><th>PO / Quotation</th><th>Invoice</th><th>Description</th><th>Before VAT</th><th>VAT %</th><th>Total VAT</th><th>Total After VAT</th><th>Credit</th><th>Balance</th><th>View</th></tr></thead><tbody>{group.invoices.length ? group.invoices.map((invoice) => { const quotation=findQuotation(invoice); const taxable=invoice.subTotal??Math.max(0,invoice.amount-(invoice.vatAmount??0)); const vat=invoice.vatAmount??Math.max(0,invoice.amount-taxable); const vatRate=invoice.vatRate??(taxable>0?vat/taxable*100:0); return <tr key={invoice.id}><td>{invoice.invoiceDate||"—"}</td><td>{invoice.quotationNo||"—"}</td><td className="strong-cell">{invoice.id}</td><td><span className="description-cell">{invoice.project||"Services"}</span></td><td className="money-cell">{money(taxable)}</td><td className="money-cell">{vatRate.toFixed(2)}%</td><td className="money-cell">{money(vat)}</td><td className="money-cell">{money(invoice.amount)}</td><td className="money-cell">{money(invoice.received)}</td><td className="money-cell">{money(group.runningBalances[invoice.id] ?? 0)}</td><td><div className="row-actions"><Link className="icon-button" href={`${routes.editInvoice}?id=${encodeURIComponent(invoice.id)}`} title="Open invoice"><ReceiptText size={15}/></Link>{quotation?<Link className="icon-button" href={`${routes.editQuotation}?id=${encodeURIComponent(quotation.id)}`} title="Open quotation"><FileText size={15}/></Link>:null}</div></td></tr>; }) : <tr className="empty-row"><td colSpan={11}>No activity in the selected date range.</td></tr>}</tbody></table></div>
