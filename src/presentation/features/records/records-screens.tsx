...
export function InvoicesScreen({pendingOnly=false}:{pendingOnly?:boolean}){
  ...
  return <><PageHeader title={pendingOnly?"Pending Payments":"Invoices & Payments"} description={pendingOnly?"Follow up outstanding balances.":"Create invoices from quotations or without a quotation, then manage payments."} actions={!pendingOnly?<><button className="button" disabled={importing} onClick={()=>input.current?.click()}>{importing?<LoaderCircle className="spin" size={14}/>:<FileUp size={14}/>}Import Excel / PDF</button><button className="button button--primary" onClick={()=>setFlowModal(true)}><Plus size={14}/>New Invoice</button></>:undefined}/>
