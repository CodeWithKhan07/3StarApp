...
export function ThemeProvider({ children }: { children: ReactNode }) {
  // Starts at the same "professional" default the server renders and the <html> tag
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      setThemeState(
        current === "dark" || current === "professional" ? current : "light"
      );
    });
  ...
  const value = useMemo<ThemeContextValue>(
    () => ({
      theme,
      toggleTheme: () =>
        applyTheme(theme === "dark" ? "light" : theme === "professional" ? "dark" : "professional"),
      setTheme: applyTheme,
    }),
    [theme, applyTheme]
  );
