...
export async function exportInvoicePdf(
  ...
  try {
    ...
    const rows = lines
      ...
      .join("");
    const emptyRows = Array.from({ length: Math.max(0, 5 - lines.length) })
      .map(
        () =>
    ...
    const supplierEmail =
      ...
      (company as CompanyProfile & { email?: string }).email ||
      "";
    const notes =
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Invoice ${escapeHtml(invoice.id)}</title><style>${sharedCss}
      .invoice{position:relative;padding:11mm 11.5mm 7mm;font-size:9pt;line-height:1.25;color:#202020;overflow:hidden}.top{display:grid;grid-template-columns:1fr 62mm;min-height:48mm}.logo{width:60mm;height:45mm;object-fit:contain}.title{text-align:right}.title h1{margin:0 0 7mm;color:#333;font-size:28pt;font-weight:400;line-height:1;letter-spacing:0}.title .number{display:block;font-size:7.2pt;color:#222;margin-bottom:7.5mm}.title p{margin:0 0 1.5mm;color:#222}.title strong{display:block;font-size:12pt;font-weight:700}.seller{margin-top:-6mm;margin-bottom:17mm;max-width:95mm;font-size:9pt;line-height:1.5}.seller b{display:block;font-size:10pt;font-weight:400;line-height:1.35}.parties{display:grid;grid-template-columns:1fr 65mm;gap:18mm;margin-bottom:13mm}.bill h3{font-size:10pt;font-weight:400;margin:0 0 4mm}.bill p{margin:0;font-size:9pt;line-height:1.5}.meta{display:grid;grid-template-columns:29mm 1fr;gap:3.9mm 5mm;font-size:9pt;align-content:start}.meta span{text-align:right;color:#222;font-size:10pt}.meta b{font-size:9pt;font-weight:400;text-align:right}.subject{margin:0 0 7mm;font-size:9pt}.subject b{display:block;margin-top:5mm;font-size:9pt;font-weight:400}.items{width:100%;border-collapse:collapse;table-layout:fixed;font-size:9pt}.items th{background:#262a2d;color:#fff;border-bottom:1px solid #262a2d;font-weight:400;text-align:center;padding:3.7mm 1.3mm;line-height:1.15}.items td{height:29mm;border-bottom:.25mm solid #d7d7d7;padding:3.2mm 1.3mm;text-align:center;vertical-align:top}.items .empty td{height:11mm}.items .c-index{width:10mm}.items .c-desc{width:70mm;text-align:left}.items .c-qty{width:23mm}.items .c-rate{width:25mm}.items .c-taxable{width:31mm}.items .c-tax-rate{width:24mm}.items .c-tax{width:24mm}.items .c-amount{width:29mm}.totals{width:70mm;margin:7mm 0 0 auto;font-size:9pt}.total-line{display:grid;grid-template-columns:1fr 28mm;gap:12mm;padding:3.3mm 0;border-bottom:.25mm solid #e3e3e3}.total-line span{text-align:right}.total-line b{text-align:right;font-weight:400}.grand{font-weight:700}.grand b{font-weight:700}.notes{margin-top:12mm;font-size:7pt;line-height:1.28}.notes h3{margin:0 0 2mm;font-size:10pt;font-weight:400}.notes p{margin:0 0 1.8mm}.zatca{margin-top:20mm;text-align:center;color:#333;font-size:9pt}.qr{width:32mm;height:32mm;image-rendering:pixelated;margin:0 auto 3mm;display:block}.powered{position:absolute;left:11.5mm;bottom:7mm;font-size:9.75pt;color:#333;letter-spacing:.1px}</style></head><body><main class="page invoice">
      ...
      <section class="seller"><b>${escapeHtml(supplierLegalName)}</b>${supplierName && supplierName !== supplierLegalName ? `<b>${escapeHtml(supplierName)}</b>` : ""}<br>${escapeHtml(supplierAddress)}<br>TRN${escapeHtml(supplierCr)} ${escapeHtml(supplierVat)}</section>
      ...
      <table class="items"><thead><tr><th class="c-index">#</th><th class="c-desc">Item &amp; Description</th><th class="c-qty">Qty</th><th class="c-rate">Rate</th><th class="c-taxable">Taxable<br>Amount</th><th class="c-tax-rate">Tax %</th><th class="c-tax">Tax</th><th class="c-amount">Amount</th></tr></thead><tbody>${rows}${emptyRows}</tbody></table>
      ...
      <section class="notes"><h3>Notes</h3><p>${escapeHtml(notes)}</p>${supplierName ? `<p>${escapeHtml(supplierName)}</p>` : ""}${supplierEmail ? `<p>${escapeHtml(supplierEmail)}</p>` : ""}${supplierPhone ? `<p>${escapeHtml(supplierPhone)}</p>` : ""}</section>
      <section class="zatca"><img class="qr" src="${qr}" alt="ZATCA QR"><p>This QR code has been generated as per ZATCA's regulations.</p></section>
